    ...
    function handleFormSubmit(event) {
        ...
        setTimeout(function() {
            try {
                ...
                const emission = Calculadora.calculateEmission(distance, transportMode);

                // Calculate car emission as baseline for comparison
                const carEmission = Calculadora.calculateEmission(distance, 'car');

                // Calculate savings compared to car (if not using car)
                const savings = transportMode !== 'car'
                    ? Calculadora.calculateSavings(emission, carEmission)
                    : null;

                // Calculate emissions for all transport modes
                const allModesComparison = Calculadora.calculateAllModes(distance);

                // Calculate carbon credits needed
                const carbonCredits = Calculadora.calculateCarbonCredits(emission);

                // Calculate estimated price for carbon credits
                const creditPrice = Calculadora.estimateCreditPrice(carbonCredits);
